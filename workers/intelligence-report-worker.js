 ...
 *
 * Gracefully skips sections that depend on tables not yet present.
 */
...
function buildStrategyHealth(db) {
  return safeQuery(db, () => db.prepare(`
...
function buildFailureAnalysis(db) {
  return safeQuery(db, () => {
    const categories = db.prepare(`
      SELECT strategy_name, failure_category, COUNT(*) AS n
      FROM loss_forensics
...
function buildAgentTrust(db) {
  ...
  `).all().map(r => ({
    ...r,
    accuracy_pct: r.recommendations > 0 ? pct(r.correct_calls, r.recommendations) : null,
  })));
}
...
function buildRecommendations(sections) {
  ...
  if (sections.calibration_status) {
    for (const c of sections.calibration_status.filter(c => c.is_degraded)) {
...
async function run() {
  ...
  try {
    ...
    const narrative = [
      ...
      `STRATEGY HEALTH`,
      critStrategies !== 'none'
        ? `Warning: Strategies needing attention: ${critStrategies}`
