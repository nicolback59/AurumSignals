...
/**
 * HYPOTHESIS ENGINE WORKER  (Prompt #12 — Phase 2)
 *
...
async function run() {
  ...
  const upsertHypothesis = db.prepare(`
    INSERT INTO research_hypotheses
      (strategy_name, hypothesis_text, dimension, condition_key, condition_value,
       ...
       priority_score, priority, status, notes, generated_at)
  ...
  for (const strategy of STRATEGIES) {
    try {
      ...
      const weeklyFreq = base.n / 52; // approximate trades per week over all history
      ...
      for (const h of hypotheses) {
        // Priority 1-5: top 20% = P1, next 20% = P2 etc.
        ...
        upsertHypothesis.run(
          ...
          h.zScore, h.pVal, h.priorityScore, priority, h.notes,
        );
        totalGenerated++;
      }

      console.log(
        `[${WORKER_NAME}] ${strategy}: ${hypotheses.length} hypotheses generated ` +
