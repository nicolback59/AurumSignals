...
CREATE TABLE IF NOT EXISTS circuit_breaker_log (
  ...
  strategy_name        TEXT    NOT NULL,
  checked_at           TEXT    NOT NULL DEFAULT (datetime('now')),
  triggered            INTEGER NOT NULL DEFAULT 0,   -- 1 = tripped, 0 = clear
  trigger_reason       TEXT,                         -- 'streak' | 'rolling_rate' | null
  streak               INTEGER,                      -- consecutive losses (most recent first)
  rolling_4h_trades    INTEGER,
  rolling_4h_loss_rate REAL,
  action_taken         TEXT                          -- 'PAUSED' | 'LIFTED' | 'NO_CHANGE'
);
